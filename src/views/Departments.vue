<template>
  <div class="container">
    ...
    <table class="table table-dark table-bordered table-hover table-responsive" @click="copyToClipboard"  v-if="filterItems.length">
      <thead>
      ...
      <tr v-for="(item, idx) in searchItems" :key="item.id">
        ...
        <td class="text-wrap-name w-100">{{ item.name }}</td>
        <td class="text-primary text-center">{{ item.id }}</td>
      </tr>
      </tbody>
    </table>
    <div class="d-ld-flex mt-3 p-3 text-danger justify-content-center align-items-center flex-column"
         v-else-if="isNotExist || !filterItems.length">Файлы пусты. Замените файлы: <strong class="text-primary">prometey.htm&nbsp;и&nbsp;prometey2.htm</strong>
...
import useDataFileDepartments from '@/use/useFileDepartments'
