    ...
    <table class="table table-dark table-bordered table-hover table-responsive table-main" @click="copyToClipboard" v-if="filterItems.length">
      ...
      <tr v-for="(item, idx) in searchItems" :key="item.id">
        ...
        <div class="d-ld-flex mt-3 p-3 text-danger justify-content-center align-items-center flex-column"
             v-else-if="isNotExist || !filterItems.length">Файл пуст. Замените файл <strong class="text-primary">&nbsp;tmc.htm&nbsp;</strong>
...
export default {
  ...
  setup () {
    ...
    const handleFileUpload = (refs) => {
      patFile.value = refs.file.files[0].path
      file.value = refs.file.files[0].name
      fileType.value = refs.file.files[0].type
      console.log('file.type', fileType.value)
      if (fileType.value === 'text/html') {
        // getDataFile(patFile.value)
        readFile()
      } else {
        errors.value.warning = 'неверный формат файла. Нужен .htm'
    ...
    console.log('isNotExist.value', isNotExist.value)
    return {
      searchItems,
      pathFoldr,
      search,
